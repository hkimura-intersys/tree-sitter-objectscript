...
- objectscript_routine/queries: expr layer + core local layer + routine local layer
...
def list_query_files() -> list[str]:
    names: set[str] = set()
    for directory in (EXPR_DIR, CORE_DIR, UDL_DIR, ROUTINE_DIR):
        if directory.exists():
...
def main() -> None:
    ...
    routine_local = {name: extract_local_layer(read_text(ROUTINE_DIR / name)) for name in filenames}

    for name in filenames:
        expr_body = expr_raw[name]
        core_body = core_local[name]
        udl_body = udl_local[name]
        routine_body = routine_local[name]
        ...
        routine_text = render_composed_file(
            name,
            [
                ("EXPR", expr_body),
                ("CORE", core_body),
        ...
        write_text(ROUTINE_DIR / name, routine_text)
